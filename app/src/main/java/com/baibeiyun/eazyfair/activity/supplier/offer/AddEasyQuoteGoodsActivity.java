...
public class AddEasyQuoteGoodsActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_easy_quote_goods);
        initYuyan();
        initView();
        initData();
    }
    ...
    private void initYuyan() {
        ...
        language = CursorUtils.selectYuYan(AddEasyQuoteGoodsActivity.this);
        if (language != null) {
    ...
    private void saved() {
        EasyQuote easyQuote = new EasyQuote();
        ...
        if (allPictureUrl != null) {
            ...
            if (i == 0) {
                ...
                easyQuote.setGoods_img1(bytes);
                easyQuote.setGoods_img2(bytes_no);
                easyQuote.setGoods_img3(bytes_no);
                easyQuote.setGoods_img4(bytes_no);
                //两张图片
            } else if (i == 1) {
                ...
                easyQuote.setGoods_img1(bytes1);
                easyQuote.setGoods_img2(bytes2);
                easyQuote.setGoods_img3(bytes_no);
                easyQuote.setGoods_img4(bytes_no);
            } else if (i == 2) {
                String[] split = allPictureUrl.split("\\|");
                ...
                easyQuote.setGoods_img1(bytes1);
                easyQuote.setGoods_img2(bytes2);
                easyQuote.setGoods_img3(bytes3);
                easyQuote.setGoods_img4(bytes_no);
            } else if (i == 3) {
                String[] split = allPictureUrl.split("\\|");
                Bitmap bitmap1 = ImageFactoryandOther.getSmallBitmap(split[0]);
                ...
                easyQuote.setGoods_img2(bytes2);
                easyQuote.setGoods_img3(bytes3);
                easyQuote.setGoods_img4(bytes4);
            }
        } else {
            easyQuote.setGoods_img1(bytes_no);
            easyQuote.setGoods_img2(bytes_no);
            easyQuote.setGoods_img3(bytes_no);
            easyQuote.setGoods_img4(bytes_no);
        }
        easyQuote.setGoods_name(input_goods_name_et.getText().toString().trim());
        easyQuote.setContent(content_et.getText().toString().trim());
        easyQuote.setUniqued_id(uniqued_id);
        String s = UUID.randomUUID().toString();
        easyQuote.setCode(s);
        EasyQuoteDao easyQuoteDao = new EasyQuoteDao(AddEasyQuoteGoodsActivity.this);
        easyQuoteDao.insertData(easyQuote);
    ...
    private void initPopupWindow() {
        ...
        //设置背景半透明
        backgroundAlpha(AddEasyQuoteGoodsActivity.this, 0.5f);
        //关闭事件
    ...
    private class popupDismissListener implements PopupWindow.OnDismissListener {
        ...
        public void onDismiss() {
            backgroundAlpha(AddEasyQuoteGoodsActivity.this, 1f);
        }
    }

    //拍照和选择相册的方法
    private void selectGallery() {
        Intent intent = new Intent(AddEasyQuoteGoodsActivity.this, MultiImageSelectorActivity.class);
        // 是否显示拍摄图片,显示
        ...
        if (requestCode == REQUEST_IMAGE) {
            if (resultCode == RESULT_OK) {
                ...
                if (mSelectPath.size() == 1 && tag.equals("2")) {
                    ...
                    GraffitiActivity.startActivityForResult(AddEasyQuoteGoodsActivity.this, params, REQ_CODE_GRAFFITI);
